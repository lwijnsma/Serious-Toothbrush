...
if(!empty($_POST['store_add'])){
	...
	if(isset($_SESSION['auth'])){
		$query="SELECT * FROM library_songs where songs_id ='".$_POST['store_add']."'and libraries_users_id = ".$_SESSION['gebruiker_informatie']['id']."";
		$result = mysqli_query($db, $query) or die("FOUT : " . mysqli_error());

		if(mysqli_num_rows($result)==0)

		{
			$query="SELECT * from cart_songs where songs_id='{$_POST['store_add']}'and cart_id ='{$_SESSION['gebruiker_informatie']['id']}'";
			$result = mysqli_query($db, $query) or die("FOUT : " . mysqli_error());
			if(mysqli_num_rows($result)==0)
			{
				$query="INSERT into cart_songs(songs_id ,cart_id) values('".$_POST['store_add']."',".$_SESSION['gebruiker_informatie']['id'].")";
